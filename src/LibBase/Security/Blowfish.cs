...
namespace LibBase.Security {
	...
	/// Разработан Брюсом Шнайером в 1993 году. Представляет собой сеть Фейстеля[1][2]. 
	/// Выполнен на простых и быстрых операциях: XOR, подстановка, сложение[2]. 
	/// Является незапатентованным и свободно распространяемым.
	/// Blowfish зарекомендовал себя, как надёжный алгоритм, поэтому реализован во многих программах, 
	/// где не требуется частая смена ключа и необходима высокая скорость шифрования/расшифровывания.[3]
	/// </summary>
	public sealed class BlowFish : IDisposable {
		...
		private void SetupKey(byte[] cipherKey) {
			...
			for (int i = 0; i < 18; i++) {
				var d =
					(uint)
						(((_key[j % cipherKey.Length] * 256 + _key[(j + 1) % cipherKey.Length]) * 256 + _key[(j + 2) % cipherKey.Length]) * 256 +
						 _key[(j + 3) % cipherKey.Length]);
				_bfP[i] ^= d;
		...
		private string ByteToHex(IEnumerable<byte> bytes) {
